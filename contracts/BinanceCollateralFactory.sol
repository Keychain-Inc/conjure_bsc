...
import "./Owned.sol";
import "./Pausable.sol";
import "./interfaces/BnbIConjure.sol";

contract BnbCollateral is ReentrancyGuard, Owned, Pausable {
    ...
    // Minimum amount of ETH to create loan preventing griefing and gas consumption. Min 0.1ETH
    uint256 public minLoanCollateralSize = SafeDecimalMath.unit() / 10;
    ...
    // ========== CONSTRUCTOR ==========
    constructor(address payable _asset, address _owner, address payable _factoryowner, uint256 _mintingfeerate ) Owned(_owner) public  {
        arbasset = _asset;
    ...
    function loanAmountFromCollateral(uint256 collateralAmount) public returns (uint256) {
        uint currentprice = BnbIConjure(arbasset).getPrice();
        uint currentbnbusdprice = uint(BnbIConjure(arbasset).getLatestBNBUSDPrice());

        return collateralAmount.multiplyDecimal(issuanceRatio()).multiplyDecimal(currentbnbusdprice).divideDecimal(currentprice);
    }
    ...
    function collateralAmountForLoan(uint256 loanAmount) public returns (uint256) {
        ...
        return
        loanAmount
        .multiplyDecimal(collateralizationRatio.divideDecimalRound(currentbnbusdprice).multiplyDecimal(currentprice))
    ...
    {
        uint256 loanAmountWithAccruedInterest = _loan.loanAmount.multiplyDecimal(BnbIConjure(arbasset).getLatestPrice());

        collateralValue = _loan.collateralAmount.multiplyDecimal(uint(BnbIConjure(arbasset).getLatestBNBUSDPrice()));
    ...
    function liquidateLoan(
        ...
        uint currentprice = BnbIConjure(arbasset).getPrice();
        uint currentethusdprice = uint(BnbIConjure(arbasset).getLatestBNBUSDPrice());
    ...
    function syntharb() internal view returns (BnbIConjure) {
        return BnbIConjure(arbasset);
    }
...
contract BnbCollateralFactory {
    event NewBnbCollateralContract(address deployed);
     ...
     */
    function BnbCollateralMint(
        address payable asset_,
        address owner_,
        address payable factoryowner_,
        uint256 mintingfeerate_
    ) public returns (address)  {
        BnbCollateral newContract = new BnbCollateral(
            asset_,
            owner_,
            factoryowner_,
            mintingfeerate_
        );
        emit NewBnbCollateralContract(address(newContract));
